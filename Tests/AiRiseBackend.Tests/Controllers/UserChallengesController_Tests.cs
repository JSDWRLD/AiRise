...
public class UserChallengesController_Tests : IDisposable
{
    ...
    public UserChallengesController_Tests()
    {
        _runner = MongoDbRunner.Start();
        var client = new MongoClient(_runner.ConnectionString);
        _db = client.GetDatabase($"UCtrl_{Guid.NewGuid()}");
        _col = _db.GetCollection<UserChallenges>("user.challenges_ctrl");
        _col.DeleteMany(FilterDefinition<UserChallenges>.Empty);

        // Requires test-only overload in the service:
        // public UserChallengesService(IMongoCollection<UserChallenges> collection) { ... }
        _svc = new UserChallengesService(_col);
    ...
    [Fact]
    public async Task CompleteToday_Ok_Returns_Doc()
    {
        // set active first
        ...
        // persisted ActiveChallengeId should be cleared
